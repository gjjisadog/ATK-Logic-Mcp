...
- **RFC 8259 JSON Escaping Contract**: Standalone CLI JSON output (`--json`) strictly escapes all strings via `atkdl16::json_escape()`, escaping `"`, `\`, control characters `0x00..0x1F`, and Windows backslashes (`\\`) to guarantee valid parsing by MCP and external tools.
- **Deep Nested HIL Schema Validation**: HIL specifications strictly validate all top-level and nested dictionary keys (`pwm_carrier`, `duty_cycle`, `deadtime`, `shoot_through_protection`, `three_phase_modulation`). Any typo or illegal bound raises `HilSpecError` fail-closed.
- **Directed Phase Sequence Verification**: Three-phase modulation validates directed phase sequence (`phase_sequence: "UVW"`). Reversed sequence ($U \to W \to V$) fails closed with `PHASE_SEQUENCE_MISMATCH`, decoupling envelope symmetry from phase sequence direction.
- **Deadband Bounds (Min & Max)**: Complementary pairs strictly enforce both `deadtime_min_ns >= min_allowed_ns` (shoot-through prevention) and `deadtime_max_ns <= max_allowed_ns` (excessive deadband prevention) across both rising and falling deadtimes.
- **Unified RX Dispatch & Order 3 Policy**: The capture engine handles interleaved packets during capture, enforcing `Order 4` ACK (`status == 3`), trimming pre-trigger samples upon receiving `Order 3`, guarding against duplicate packets, and validating full sample depth before completion.
- **Stream Write-State Validation**: Artifact storage rigorously checks file stream status (`out.good() && !out.fail() && !out.bad()`) across all binary `.bits`, `edges.json`, and `meta.json` files, failing closed with `ErrorCode::ArtifactWriteError` on any I/O error.
...
- **Center-Aligned ePWM Physical Semantics**: In center-aligned symmetric PWM (e.g. TI TMS320F28P65 Up-Down count), output edge distance between phases shifts with duty cycle. Carrier phase is reported as `"NOT_MEASURED"` across output pins; carrier phase synchronization requires a dedicated reference channel (e.g. EPWM11 / CarrierSync). Output switching jitter under dynamic sinusoidal modulation is tracked as output-edge period variation RMS (`max_output_edge_period_variation_rms_ns`).
